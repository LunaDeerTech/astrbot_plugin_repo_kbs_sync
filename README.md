...
知识库文档名使用仓库相对路径，例如 `产品文档/入门.mdx`。MDX 文件在预处理为 Markdown 后，上传给 AstrBot 的文档名会改用 `.md` 后缀（如 `产品文档/入门.md`），因为 AstrBot 的 markitdown 解析器只依据文件名后缀推断格式、且不支持 `.mdx`；插件内部的源路径仍保留 `.mdx`，用于 Git 改名/删除追踪。`file_type` 在 MDX 预处理开启时统一为 `md`。

注意：若同一目录下同时存在 `foo.md` 与 `foo.mdx`，两者会被映射到相同的文档名 `foo.md`，后者会覆盖前者；请避免在同一映射路径下使用同名异后缀的文档。
